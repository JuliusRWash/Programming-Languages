...
{
  if ( userinput[pos] == '-')  && isDigit[pos+1] )
      return 1;
   if ( userinput[pos] == '+')  && isDigit[pos+1] )
      return 1;
    return isDigit[pos];
...
VALUELIST evalList ( EXPLIST el, ENV rho)
{
    ...
    VALUELIST v = evealList(e->tail, rho);
...
NUMBER applyCtrlOp ( int op,  EXPLIST args, ENV rho )
{
   ...
   if (op == 0)  { // (if  e1  e2  e3 )
      ...
      if isTruval(eval(args->head,rho))
   ...
   if( op == 2)
    {
	   ...
	   else bindVar(x, n, globalEnv);
	   return n;
    }
   if (op == 3) {  //(begin e1, e2, .....en)
     evalaue e1, e2, ....en,  return value of en
   }
}// applyCtrlOp
